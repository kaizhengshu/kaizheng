...
@interface XboxOneViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    //数据源
    NSMutableArray * _XboxOneData;
...
@implementation XboxOneViewController
...
{
    //解析数据
    _XboxOneData =  [NetWorkManager initXmlWithUrl:XboxUrl success:nil faile:nil];
    //如果无数据 报告
    if (_XboxOneData == nil){
    ...
    if (isRemoveAll == 1){ // 下拉刷新
        ...
        _titleArr = _XboxOneData[0];
        _detailArr = _XboxOneData[1];
        _iconArr = _XboxOneData[2];
        //遍历数组
